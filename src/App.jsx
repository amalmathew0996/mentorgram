        ...
        <div style={S.grid2}>
          {paginated.map((j, i) => (
            <div key={i}
              ...
              style={{ ...S.card, display: "flex", flexDirection: "column", justifyContent: "space-between", cursor: "pointer", transition: "box-shadow 0.2s, border-color 0.2s", minHeight: "180px" }}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "8px", marginBottom: "6px" }}>
                ...
                <span style={{ flexShrink: 0, display: "inline-block", padding: "3px 8px", borderRadius: "var(--border-radius-md)", fontSize: "11px", fontWeight: 500,
              ...
              {/* MIDDLE: Tags + location + date */}
              <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", alignItems: "center", marginBottom: "8px" }}>
                {j.sector && <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: "var(--border-radius-md)", fontSize: "11px", fontWeight: 500, background: "rgba(26,63,168,0.12)", color: "#1A3FA8" }}>{j.sector}</span>}
                {j.source === "jobs.ac.uk" && <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: "var(--border-radius-md)", fontSize: "11px", fontWeight: 500, background: "rgba(26,63,168,0.08)", color: "#1A3FA8" }}>🎓 jobs.ac.uk</span>}
                <span style={{ fontSize: "12px", color: "var(--color-text-secondary)" }}>📍 {j.location}</span>
                {j.posted && <span style={{ fontSize: "12px", color: "var(--color-text-secondary)" }}>📅 {j.posted}</span>}
              </div>

              {/* BOTTOM: Salary + action buttons — always pinned to bottom */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "8px", marginTop: "auto" }}>
                <p style={{ fontWeight: 600, color: "var(--color-text-primary)", margin: 0, fontSize: "13px", flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{j.salary || "Competitive"}</p>
                <div style={{ display: "flex", gap: "6px", flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                  <ShareButton job={j} />
                  <button
                    onClick={e => { e.stopPropagation(); setClickedJob(i); setTimeout(() => { onSelectJob(j); setClickedJob(null); }, 320); }}
                    style={{ padding: "6px 14px", borderRadius: "var(--border-radius-md)", background: "#1A3FA8", color: "#fff", fontSize: "12px", fontWeight: 500, border: "none", cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>
                    View ↗
                  </button>
                </div>
              </div>
